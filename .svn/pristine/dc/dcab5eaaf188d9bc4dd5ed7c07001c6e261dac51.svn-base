...
export async function createInfoData(name, parmse){
    ...
    //单位基本情况
    if(name == 'UnitInfo'){
        res = await http.post("agricultural/ty/task/createCompany", parmse.agriculturalCompany)
    }
    //财务状况
    if(name == 'InancialInfo'){
        res = await http.post("agricultural/ty/task/createFinance", parmse.agriculturalFinance)
    }
    //人员基本情况
    if(name == 'PersonInfo'){
        let Share = await http.post("agricultural/ty/task/createShareholder", parmse.agriculturalShareholder)
        if(Share.status == 200){
            res = await http.post("agricultural/ty/task/createPers", parmse.agriculturalPers)
        }
    }
    //股东情况
    ...
    //支持情况
    if(name == 'HoldInfo'){
        res = await http.post("agricultural/ty/task/createSupport", parmse.agriculturalSupport)
    }
    //贷款情况
    if(name == 'CountableInfo'){
        res = await http.post("agricultural/ty/task/createLoan", parmse.agriculturalLoan)
    }
