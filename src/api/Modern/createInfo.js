...
export async function createInfoData(name, parmse){
    ...
    if(name == 'UnitInfo'){
        ...
        res = await http.post("modern/ty/apply/createCompany", parmse.modernCompany)
        if(res.status == 200){
            res = await http.post("modern/ty/apply/createShareholders", parmse.modernShareholders)
        }
    }
    ...
    if(name == 'InancialInfo'){
        if(parmse.modernCompany.regist_type == '企业'){
            console.log('企业类')
            res = await http.post("modern/ty/apply/createFinances", parmse.modernFinances)
        }
        if(parmse.modernCompany.regist_type == '事业单位'){
            console.log('事业单位')
            res = await http.post("modern/ty/apply/createCauses", parmse.modernCauses)
        }
    }

    //支持情况
    if(name == 'HoldInfo'){
        res = await http.post("modern/ty/apply/createSupport", parmse.modernSupport)
    }
    //项目实施的背景、意义及实施内容（限1500字之内)
    if(name == 'ProjectContent'){
        res = await http.post("modern/ty/apply/createImplement", parmse.modernImplement)
    }
    ...
    if(name == 'ProjectInvest'){
        ...
        console.log(parmse.modernInvestMent, parmse.modernEquipment)
            ...
            res = await http.post("modern/ty/apply/createInvestMent", parmse.modernInvestMent)
        if(res.status == 200){
            //设备
            res = await http.post("modern/ty/apply/createEquipment", parmse.modernEquipment)
        }
    }

    if(name == 'SummarizeInfo'){
        res = await http.post("modern/ty/apply/createAbstart", parmse.modernAbstract)
