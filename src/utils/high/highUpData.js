...
export function highData(newData) {
  ...
  for (let item in newData) {
    ...
    if (item == "techCompany") {
      newData["basic_info"] = {
        basic_info: newData[item].basic_info,
      };
    }
    if (item == "techStageList" && newData[item].length > 0) {
      for (let res of newData[item]) {
        res["stage"] = [res.stage_start, res.stage_end];
      }
    }

    if (item == "techFundsList" && newData[item].length > 0) {
      let techFundAMT = {
        total_invest: newData[item][0].total_invest,
        state_funding: newData[item][0].state_funding,
